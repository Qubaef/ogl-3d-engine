...
float tessLevelDist(vec3 cameraPos, int coordId1, int coordId2)
{
	...
	p1.y = texture(terrainHeight, vec2(p1.x, p1.z) / terrainSize + 0.5).x + terrainHeightOffset;
	p2.y = texture(terrainHeight, vec2(p2.x, p2.z) / terrainSize + 0.5).x + terrainHeightOffset;
...
float tessLevelSphere(int coordId1, int coordId2)
{
	...
	float d = distance(screen1, screen2);
	float t = clamp(floor(d / triangleTargetWidth), 0, 64);
