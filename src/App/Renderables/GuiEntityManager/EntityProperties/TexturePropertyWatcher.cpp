...
void TexturePropertyWatcher::display()
{
	...
	{
		const ImVec2 uv_min = ImVec2(1.0f, 1.0f);						// Top-left
		const ImVec2 uv_max = ImVec2(0.0f, 0.0f);						// Lower-right
		const ImVec4 tint_col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);	// No tint
		const ImVec4 border_col = ImVec4(1.0f, 1.0f, 1.0f, .5f);	// 50% opaque white
