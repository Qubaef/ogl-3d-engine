...
protected:
	...
	_Terrain(int start_pos_x, int start_pos_z, int terrain_size, int vertices_number, Shader* p_shader, CameraController* p_camera_controller);

	// Each _Terrain should provide it's own implementation of initialize_OGL_objects method
	...
	// Each _Terrain should provide it's own implementation of generate_terrain method
	...
	// Each _Terrain should provide it's own implementation of prepare_data method
...
public:
	// Each _Terrain should provide it's own implementation of render_terrain method
	// render_terrain will be called each frame, so it should update ogl buffers and call draw functions for them
	// _Terrain delivers default render method, which gets and sets uniforms for vertex shader (MVP)
